...
function calculateStateFromDb(db) {
  ...
  let fundTotalDeposit = ZERO;
  let fundTotalWithdraw = ZERO;
  let fundCnhDeposit = ZERO;
  let fundCnhWithdraw = ZERO;
  Object.values(members).forEach(member => {
    fundTotalDeposit = fundTotalDeposit.plus(member.totalDeposit);
    fundTotalWithdraw = fundTotalWithdraw.plus(member.totalWithdraw);
    fundCnhDeposit = fundCnhDeposit.plus(member.cnhDeposit);
    fundCnhWithdraw = fundCnhWithdraw.plus(member.cnhWithdraw);
  });

  const fundProfit = totalNAV.plus(fundTotalWithdraw).minus(fundTotalDeposit);
  const fundProfitRate = fundTotalDeposit.isZero() ? ZERO : fundProfit.div(fundTotalDeposit).mul(100);
  const fundCnhCurrentValue = totalNAV.mul(globalCnhRate);
  const fundCnhProfit = fundCnhCurrentValue.plus(fundCnhWithdraw).minus(fundCnhDeposit);
  const fundCnhProfitRate = fundCnhDeposit.isZero() ? ZERO : fundCnhProfit.div(fundCnhDeposit).mul(100);
  ...
  return {
    summary: {
      ...
      totalShares: output(totalShares, 4),
      navPerShare: output(navPerShare, 4),
      totalDeposit: output(fundTotalDeposit, 2),
      totalWithdraw: output(fundTotalWithdraw, 2),
      ...
      cnhTotalDeposit: output(fundCnhDeposit, 2),
      cnhTotalWithdraw: output(fundCnhWithdraw, 2),
