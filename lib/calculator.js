  ...
  sortedEvents.forEach(event => {
    ...
    if (event.type === 'deposit') {
      ...
      if (member) {
        ...
        const fullExitTolerance = Decimal.max(decimal('0.000001'), requestAccountValue.mul('1e-10'));
        const isFullExit = event.fullExit === true || (
          autoFullExitEventIds.has(event.id) && amount.minus(requestAccountValue).abs().lte(fullExitTolerance)
        );
        ...
        if (isFullExit) lpSharesDeducted = lpSharesBefore;
      ...
      if (fromMember) {
        ...
        const lpSharesBefore = fromMember.lpShares;
        const accountValueBefore = sharesBefore.mul(currentNAV);
        const requestAccountValue = accountValueBefore.toDecimalPlaces(2);
        const fullExitTolerance = Decimal.max(decimal('0.000001'), requestAccountValue.mul('1e-10'));
        const isFullExit = event.fullExit === true || (
        ...
        const usesNetDisposal = Number(feeConfig?.disposalVersion || 1) >= 2;
        if (isFullExit) lpSharesTransferred = lpSharesBefore;
        const disposal = gpMember ? previewDisposalFee({
          member: fromMember,
          currentNAV,
          date: event.date,
        ...
        if (!lpSharesBefore.isZero()) {
          if (gpMember) {
            fromMember.lots = disposal.remainingLots;
            fromMember.lpShares = disposal.remainingLpShares;
          } else {
            fromMember.lpShares = takeLotsProRata(fromMember.lots, isFullExit ? ONE : disposal.ratio);
          }
        }
        const carryTransferred = sharesTransferred.minus(lpSharesTransferred);
        fromMember.carryShares = isFullExit
          ? ZERO
          : Decimal.max(ZERO, fromMember.carryShares.minus(carryTransferred));
        fromMember.shares = isSelfGp && isFullExit
