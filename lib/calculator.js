...
const { hurdleValue, settlePerformance, previewDisposalFee } = require('./performance-settlement');
...
function takeLotsProRata(lots, shareRatio) {
  ...
  for (const lot of lots) {
    ...
    if (!shares.isZero()) moved.push({
      shares, basis, date: lot.date,
...
// Historical (unversioned) partial disposals removed the requested cash
// shares pro rata, then removed each lot's carry shares on top. Keep this only
// for deterministic replay of existing ledgers; new events use version 2.
function takeLotsForLegacyDisposal(lots, disposalRatio, feeSharesByLot = []) {
  const moved = [];
  lots.forEach((lot, index) => {
    const cashShares = lot.shares.mul(disposalRatio);
    const feeShares = feeSharesByLot[index] || ZERO;
    const shares = Decimal.min(lot.shares, cashShares.plus(feeShares));
    const basis = lot.shares.isZero() ? ZERO : lot.basis.mul(shares.div(lot.shares));
    lot.shares = lot.shares.minus(shares);
    lot.basis = lot.basis.minus(basis);
    if (!shares.isZero()) moved.push({
      shares, basis, date: lot.date,
  ...
  sortedEvents.forEach(event => {
    ...
    if (event.type === 'deposit') {
      ...
      if (member) {
        ...
        const lpSharesBefore = member.lots.reduce((sum, lot) => sum.plus(lot.shares), ZERO);
        ...
        let lpSharesDeducted = Decimal.min(sharesDeducted, lpSharesBefore);
        ...
        const usesNetDisposal = Number(feeConfig?.disposalVersion || 1) >= 2;
        if (event.fullExit === true) lpSharesDeducted = lpSharesBefore;
        const disposal = gpMember ? previewDisposalFee({
          member,
          currentNAV,
          date: event.date,
          lpSharesDisposed: lpSharesDeducted,
          netCashAmount: event.fullExit === true || !usesNetDisposal
            ? undefined
            : lpSharesDeducted.mul(currentNAV),
          annualRate: decimal(feeConfig.annualRate),
          feeRate: decimal(feeConfig.feeRate),
          output
        }) : {
          fee: ZERO,
          feeShares: ZERO,
          cashShares: lpSharesDeducted,
          ratio: lpSharesBefore.isZero() ? ZERO : lpSharesDeducted.div(lpSharesBefore),
          lots: []
        };
        if (usesNetDisposal && event.fullExit !== true) {
          lpSharesDeducted = Decimal.min(lpSharesBefore, disposal.cashShares);
        }
        const feeShares = event.fullExit === true
          ? Decimal.min(disposal.feeShares, lpSharesBefore)
          : Decimal.min(disposal.feeShares, Decimal.max(ZERO, lpSharesBefore.minus(lpSharesDeducted)));
        if (event.fullExit === true) {
          sharesDeducted = isSelfGp
            ? sharesBefore
            : lpSharesBefore.minus(feeShares).plus(carrySharesBefore);
          actualAmount = sharesDeducted.mul(currentNAV);
          eventCnhAmount = amount.isZero() ? ZERO : eventCnhAmount.mul(actualAmount.div(amount));
        }
        const carrySharesDisposed = event.fullExit === true
          ? carrySharesBefore
          : Decimal.max(ZERO, sharesDeducted.minus(lpSharesDeducted));
        const grossAmount = event.fullExit === true ? sharesBefore.mul(currentNAV) : actualAmount;
        // A full exit disposes every LP lot exactly once.  The performance-fee
        // shares are carved out of that same position, so adding them again to
        // lpSharesDeducted would produce a ratio above 1 and negative lots.
        if (!lpSharesBefore.isZero()) {
          if (event.fullExit === true || usesNetDisposal) {
            takeLotsProRata(member.lots, event.fullExit === true ? ONE : disposal.ratio);
          } else {
            takeLotsForLegacyDisposal(
              member.lots,
              lpSharesDeducted.div(lpSharesBefore),
              disposal.feeSharesByLot
            );
          }
        }
        if (event.fullExit === true) {
          member.carryShares = ZERO;
        } else {
          member.carryShares = Decimal.max(ZERO, member.carryShares.minus(sharesDeducted.minus(lpSharesDeducted)));
        }
        member.shares = isSelfGp && event.fullExit === true
          ? member.shares.minus(sharesDeducted)
          : member.shares.minus(sharesDeducted.plus(feeShares));
        if (gpMember && feeShares.gt(ZERO) && !(isSelfGp && event.fullExit === true)) {
        ...
        event._grossAmount = output(grossAmount, 12);
        event._performanceFee = output(disposal.fee, 12);
        event._performanceFeeShares = output(feeShares, 12);
        event._carrySharesDisposed = output(carrySharesDisposed, 12);
        event._unpaidPerformanceFeeShares = output(disposal.feeShares.minus(feeShares), 12);
        event._fullExit = event.fullExit === true;
        event._disposalVersion = usesNetDisposal ? 2 : 1;
        ...
        event._disposedLots = disposal.lots;
      }
      totalNAV = totalShares.mul(currentNAV);
      navPerShare = currentNAV;
      ...
      if (fromMember) {
        const sharesBefore = fromMember.shares;
        const carrySharesBefore = fromMember.carryShares;
        const lpSharesBefore = fromMember.lots.reduce((sum, lot) => sum.plus(lot.shares), ZERO);
        ...
        let lpSharesTransferred = Decimal.min(sharesTransferred, lpSharesBefore);
        const feeConfig = event.performanceFee;
        const gpMember = feeConfig ? members[feeConfig.gpMember] : null;
        const isSelfGp = gpMember === fromMember;
        const usesNetDisposal = Number(feeConfig?.disposalVersion || 1) >= 2;
        if (event.fullExit === true) lpSharesTransferred = lpSharesBefore;
        const disposal = gpMember ? previewDisposalFee({
          member: fromMember,
          currentNAV,
          date: event.date,
          lpSharesDisposed: lpSharesTransferred,
          netCashAmount: event.fullExit === true || !usesNetDisposal
            ? undefined
            : lpSharesTransferred.mul(currentNAV),
          annualRate: decimal(feeConfig.annualRate),
          feeRate: decimal(feeConfig.feeRate),
          output
        }) : {
          fee: ZERO,
          feeShares: ZERO,
          cashShares: lpSharesTransferred,
          ratio: lpSharesBefore.isZero() ? ZERO : lpSharesTransferred.div(lpSharesBefore),
          lots: []
        };
        if (usesNetDisposal && event.fullExit !== true) {
          lpSharesTransferred = Decimal.min(lpSharesBefore, disposal.cashShares);
        }
        const feeShares = event.fullExit === true
          ? Decimal.min(disposal.feeShares, lpSharesBefore)
          : Decimal.min(disposal.feeShares, Decimal.max(ZERO, lpSharesBefore.minus(lpSharesTransferred)));
        if (event.fullExit === true) {
          sharesTransferred = isSelfGp
            ? sharesBefore
            : lpSharesBefore.minus(feeShares).plus(carrySharesBefore);
          actualAmount = sharesTransferred.mul(currentNAV);
          lpSharesTransferred = sharesTransferred;
          eventCnhAmount = amount.isZero() ? ZERO : eventCnhAmount.mul(actualAmount.div(amount));
        }
        const carrySharesDisposed = event.fullExit === true
          ? carrySharesBefore
          : Decimal.max(ZERO, sharesTransferred.minus(lpSharesTransferred));
        const grossAmount = event.fullExit === true ? sharesBefore.mul(currentNAV) : actualAmount;
        const netCnhAmount = eventCnhAmount;
        const netSharesTransferred = sharesTransferred;
        if (!lpSharesBefore.isZero()) {
          if (event.fullExit === true || usesNetDisposal) {
            takeLotsProRata(fromMember.lots, event.fullExit === true ? ONE : disposal.ratio);
          } else {
            takeLotsForLegacyDisposal(
              fromMember.lots,
              lpSharesTransferred.div(lpSharesBefore),
              disposal.feeSharesByLot
            );
          }
        }
        const carryTransferred = sharesTransferred.minus(lpSharesTransferred);
        fromMember.carryShares = event.fullExit === true
          ? ZERO
          : Decimal.max(ZERO, fromMember.carryShares.minus(carryTransferred));
        fromMember.shares = isSelfGp && event.fullExit === true
          ? fromMember.shares.minus(sharesTransferred)
          : fromMember.shares.minus(sharesTransferred.plus(feeShares));
        if (gpMember && feeShares.gt(ZERO) && !(isSelfGp && event.fullExit === true)) {
          gpMember.shares = gpMember.shares.plus(feeShares);
          gpMember.carryShares = gpMember.carryShares.plus(feeShares);
        }
  ...
  Object.keys(members).forEach(k => {
    ...
    computedMembers[k] = {
      ...
      ,lpLedger: member.lots.map(lot => ({
        ...
        hurdle: output(ledgerDate ? hurdleValue(lot, ledgerDate, decimal(0.06)) : lot.basis, 2),
    ...
    charts: {
      ...
      benchmarkAnchors: Object.fromEntries(
        Object.entries(indexCache)
          ...
          .map(([date, entry]) => [date.slice(0, 4), entry])
      )
    }
  };
}
