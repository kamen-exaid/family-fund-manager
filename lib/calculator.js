    ...
    if (event.type === 'deposit') {
      ...
      if (member) {
        ...
        if (event.fullExit === true) {
          sharesDeducted = lpSharesBefore.minus(feeShares);
          actualAmount = sharesDeducted.mul(currentNAV);
          eventCnhAmount = amount.isZero() ? ZERO : eventCnhAmount.mul(actualAmount.div(amount));
        }
        const grossAmount = event.fullExit === true ? lpSharesBefore.mul(currentNAV) : actualAmount;
        ...
        if (!lpSharesBefore.isZero()) {
          const lotRatio = event.fullExit === true
            ? ONE
            : lpSharesDeducted.plus(feeShares).div(lpSharesBefore);
          takeLotsProRata(member.lots, lotRatio);
        }
        member.carryShares = Decimal.max(ZERO, member.carryShares.minus(sharesDeducted.minus(lpSharesDeducted)));
        member.shares = member.shares.minus(sharesDeducted.plus(feeShares));
        if (gpMember && feeShares.gt(ZERO)) {
      ...
      if (fromMember) {
        ...
        if (event.fullExit === true) {
          sharesTransferred = lpSharesBefore.minus(feeShares);
          actualAmount = sharesTransferred.mul(currentNAV);
        ...
        if (!lpSharesBefore.isZero()) takeLotsProRata(fromMember.lots, lpSharesTransferred.plus(feeShares).div(lpSharesBefore));
        const carryTransferred = sharesTransferred.minus(lpSharesTransferred);
        fromMember.carryShares = Decimal.max(ZERO, fromMember.carryShares.minus(carryTransferred));
        fromMember.shares = fromMember.shares.minus(sharesTransferred.plus(feeShares));
        if (gpMember && feeShares.gt(ZERO)) {
          gpMember.shares = gpMember.shares.plus(feeShares);
          gpMember.carryShares = gpMember.carryShares.plus(feeShares);
        }
