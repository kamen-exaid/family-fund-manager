...
function isAtOrBefore(event, cutoff) {
  if (event.date !== cutoff.date) return event.date < cutoff.date;
  return Number(event.createdAt) <= Number(cutoff.createdAt);
}
...
function migrateSettlementLedger(baseDb, ledger) {
  ...
  for (const original of ledger.records) {
  ...
  return {
    ledger: { version: 1, records: processedRecords },
