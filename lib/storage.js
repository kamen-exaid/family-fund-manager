...
function readSettlements() {
  ...
  try {
    ...
    if (data?.version !== 1 || !Array.isArray(data.records)) throw new Error('invalid settlements format');
...
function writeSettlements(data) {
  ensureDataDirs();
  const normalized = { version: 1, records: Array.isArray(data?.records) ? data.records : [] };
