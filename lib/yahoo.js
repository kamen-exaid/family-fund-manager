...
async function fetchTickerAthData(config) {
  ...
  async function fetchSingleTickerAth(ticker) {
    try {
      ...
      // 今年涨幅以美东当前年份为准，基准为上一年最后一个交易日的收盘价。
      const yearStart = `${estMap.year}-01-01`;
      let previousYearClose = null;
      for (let i = closes.length - 1; i >= 0; i--) {
        if (closes[i] === null || closes[i] === undefined) continue;
        const dateStr = new Date(timestamps[i] * 1000).toISOString().split('T')[0];
        if (dateStr < yearStart) {
          previousYearClose = closes[i];
          break;
        }
      }
...
module.exports = { fetchYahooPrices, findCloseForPolicy, findPreviousClose, fetchTickerAthData, fetchCnhRateFromApi };
